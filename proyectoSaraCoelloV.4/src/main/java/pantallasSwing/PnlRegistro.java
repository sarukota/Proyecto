...
public class PnlRegistro extends javax.swing.JPanel {
    ...
    Vehiculo vehiculo;
    Alerta alerta;
    private int contadorCliente; //Contador para cambiar lbl clientes
            ...
            if (cliente != null) {
                try {
                    ...
                    if ((contadorCliente) >= vehiculo.getNumOcupantes()){
                        cambiarPnl();
                        JOptionPane.showMessageDialog(null,"El vehiculo "+vehiculo.getMatricula()+" y todos sus ocupantes han sido guardados correctamente","Aviso",1);
                    }
                    contadorCliente++;
                    
                } catch (SQLException ex) {
                    JOptionPane.showMessageDialog(this, "El cliente no ha podido añadirse","Error",0);
                }
        ...
        if (vehiculo != null) { // Si los datos son correctos
            ...
            try {
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(this, "No se ha podido añadir la alerta del vehículo","Error",0);
                }
                
            } catch (SQLException ex) {
                if (ex.getMessage().toLowerCase().contains("duplicate")) { // Si el mensaje es entrada duplicada, es porque se repite la matrícula
                    JOptionPane.showMessageDialog(this, "La matrícula ya está en el sistema.","Error",0);
                }
                else {
                    JOptionPane.showMessageDialog(this, "No se ha podido añadir el vehículo","Error",0);
                }
            }
