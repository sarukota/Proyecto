...
public class PnlRegistro extends javax.swing.JPanel {
    ...
    Vehiculo vehiculo;
    Alerta alerta;
    int contador; //Contador para cambiar label de clientes
    ...
    public void limpiarCampos(JTextField tf[]){
    ...
    private void btnGuardarClienteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarClienteActionPerformed
        ...
        if(tfDNI.getText().length()==0|tfNombre.getText().length()==0|tfApellido1.getText().length()==0|tfTelefono.getText().length()==0){
            ...
            lblCliente.setText("Cliente "+(contador+1));
            if ((contador) >= vehiculo.getNumOcupantes()){
                try {
                    //Se añaden el vehiculo, su alerta y sus ocupantes a la BD y se cambia la disponibilidad de la parcela correspondiente
                    ...
                    for (Cliente clienteBd: clientes) {
                        conexion.updateBd(clienteBd.toSQL());
                    }
