...
public class Interfaz extends javax.swing.JFrame {
    ...
    public void ConfigInicial() throws SQLException{    
        ...
        /*Si existen datos en tabla Area de la BD: se liberan las parcelas de los vehiculos que salen el día de hoy 
          si hay datos en la tabla vehiculos y me lleva a la pantalla alertas*/
            ...
            String consulta = "SELECT* FROM alertas WHERE dia_alerta LIKE '"+conexion.fechaSQL(fechaHoy)+"';";
                try {
                    if(conexion.tieneDatos(consulta)){
                        int alertasHoy = Integer.parseInt(conexion.selectDato("SELECT count(*) FROM alertas WHERE dia_alerta LIKE '"+conexion.fechaSQL(fechaHoy)+"';"));
                        if (alertasHoy == 1) JOptionPane.showMessageDialog(pA,"Tiene "+alertasHoy+" alerta para hoy","Aviso",2);
                        else JOptionPane.showMessageDialog(this,"Tiene "+alertasHoy+" alertas para hoy","Aviso",2);
                    }
                } catch (SQLException ex) {
                    Logger.getLogger(PnlAlertas.class.getName()).log(Level.SEVERE, null, ex);
                    JOptionPane.showMessageDialog(this,"Fallo en la comprobación de alertas","Error",0);
                }
        }
    ...
    public boolean comprobacionDatosArea() {
        ...
        if (telefono.isBlank()) { // Comprobar el precio vacío
            tfTelefonoArea.setBackground(Color.red);
            datosCorrectos = false;
        }
        else {
            datosCorrectos = patternInteger.matcher(telefono).matches(); // Comprobar que el precio es un int
    ...
    public Area crearArea(){
        ...
        if (comprobacionDatosArea()) { //Si los datos son correctos
    ...
    public boolean comprobacionDatosServicios() {
        ...
        if (precio.isBlank()) {
            ...
            datosCorrectos = patternInteger.matcher(precio).matches(); // Comprobar que el precio es un int
            if (!datosCorrectos) {
            tfPrecio.setBackground(Color.red);
            }
        }
    ...
    public void setMdlTblServicios(){
    ...
    public void llenarArrayServicios(){
        String nombre;
    ...
    int contador = 0;
        for (Servicio srv : listaServicios) {
            ...
            try {
                ...
                while(contador==0){
                    contador++;
                    JOptionPane.showMessageDialog(this,"Se han actualizado los datos con éxito","Aviso",1);
                }
            } catch (SQLException ex) {
                if (ex.getMessage().toLowerCase().contains("duplicate")) { // Si el mensaje es entrada duplicada, es porque se repite la matrícula
                    JOptionPane.showMessageDialog(this, "Ya existe un servicio con ese título","Aviso",1);
                }
                Logger.getLogger(Interfaz.class.getName()).log(Level.SEVERE, null, ex);
                JOptionPane.showMessageDialog(this,"Los servicios no se han podido añadir.","Error",0);
            }
        }
    }
    
    //metodo para mostrar los datos de los servicios en la Jtable de la interfaz
    public void llenarTblServicios(){
    ...
    public void insertArea(){
        ...
        if (miArea != null) {
            try {
                ...
                Logger.getLogger(Interfaz.class.getName()).log(Level.SEVERE, null, ex);
                JOptionPane.showMessageDialog(this,"No se ha podido insertar el área.","Error",0);
            }
        }
    }
    ...
    public void updateArea() throws SQLException{
        ...
        //Si las parcelas del area antigua son mas que las de la nueva,las parcelas sobrantes se eliminarán solo si estan vacias
            ...
            if (contador > 0){
                JOptionPane.showMessageDialog(this,"Algunas de las parcelas que se quieren eliminar contienen vehículos.\nCambialos de parcela o eliminalos para proceder.","Aviso",2);
            }else{
    ...
    private void btnAnadirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAnadirActionPerformed
        if (comprobacionDatosServicios()){
            llenarArrayServicios();
            llenarTblServicios();
            tfPrecio.setText("");
        }else{
            JOptionPane.showMessageDialog(this, "Compruebe los datos.","Error",0);
    ...
    private void btnGuardarAreaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarAreaActionPerformed
        ...
        if (comprobacionDatosArea()) {
            try {
                ...
                Logger.getLogger(Interfaz.class.getName()).log(Level.SEVERE, null, ex);
                JOptionPane.showMessageDialog(this, "El área no ha podido insertarse.","Error",0);
            } 
        }
        else {
            JOptionPane.showMessageDialog(this, "Compruebe los datos.","Error",0);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null,"Fallo al conectar con la base de datos","Error",0);
            Logger.getLogger(Interfaz.class.getName()).log(Level.SEVERE, null, ex);
        }
